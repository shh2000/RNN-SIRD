...
class EnDeModel(torch.nn.Module):
    ...
    def forward(self, history_u, sird):
        ...
        for step in range(self.future_u_length + self.target_step - 1):
        ...
        x_foo = [x]
        for days in range(self.target_step - 1):
            s, i, r, d, n = torch.split(x, 1, dim=2)
            si = s * i * betat[days + self.future_u_length] / n
            ir = i * self.recover_rate
            id = i * self.death_rate
            r = r + ir
            d = d + id
