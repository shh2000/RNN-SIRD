...
class EnDeModel(torch.nn.Module):
    def __init__(self, recover_rate, death_rate, history_length, future_u_length, hidden_size):
        ...
        self.enc_fc = torch.nn.Linear(self.history_length, self.hidden_size)
        self.rnn = torch.nn.GRU(input_size=self.hidden_size, hidden_size=self.hidden_size, num_layers=1)
        self.linear_after_rnn = torch.nn.Linear(self.hidden_size, 1)

    def forward(self, history_u, sird, target_step):
        h = self.enc_fc(history_u)
        ...
        for step in range(self.future_u_length):
            y, h = self.rnn(y, h)
            out = torch.sigmoid(self.linear_after_rnn(y))
            betat.append(out)
